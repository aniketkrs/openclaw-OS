...
export function createSessionHistoryWorkerReaders(
  ...
  return {
    ...
    readExactEntries: async (input, signal) => {
      // input.env may be a Proxy from cloneEnvWithPlatformSemantics (Windows).
      // worker.postMessage rejects Proxies with DataCloneError, so capture a
      // plain object first (same pattern as readRowFacts below).
